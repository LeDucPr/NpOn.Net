...
using ObjectHandlerFlow.AlgObjs.CtrlObjs.Data;
using ObjectHandlerFlow.AlgObjs.SqlQueries;

namespace HandleFlow.ResultConverters.Domain;
...
public static class JoiningTableExtensions
{
        ...
        INpOnWrapperResult unifiedTableMappingDecoys)
    {
        ...
        List<SysBaseCtrl>? ctrls = unifiedTableMappingDecoys.SysGenericConverter(typeof(UnifiedTableMappingCtrl))?.ToList();
        ...
        Func<TableCtrl, List<UnifiedTableMappingCtrl>?, Task<INpOnWrapperResult?>> getDataFunc =
            async (table, groupedMappings) =>
            {
                ...
                INpOnWrapperResult? getDataResult = await factoryWrapper.ExecuteAsync(queryWithFieldAndTableObject);
