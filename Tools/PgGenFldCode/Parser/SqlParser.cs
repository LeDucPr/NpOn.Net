...
namespace Tools.PgGenFldCode.Parser;
...
public class BooleanLiteral : SelectItem
{
...
public class SqlParser
{
    ...
    private List<SelectItem> ParseSelectList(string selectSql)
    {
        ...
        foreach (var raw in items)
        {
            ...
            // 3.5 Boolean literal: true/false [AS alias]
            var boolMatch = Regex.Match(s,
                @"^(?<val>true|false)(?:\s+(?:AS\s+)?(?<alias>[A-Za-z_][A-Za-z0-9_]*))?$",
                RegexOptions.IgnoreCase);
            if (boolMatch.Success)
            {
                list.Add(new BooleanLiteral
                {
                    Value = boolMatch.Groups["val"].Value.Equals("true", StringComparison.OrdinalIgnoreCase),
                    Alias = boolMatch.Groups["alias"].Success ? boolMatch.Groups["alias"].Value : null
                });
                continue;
            }
            ...
            // 5.5 Expression with alias: expr AS alias
            var exprAliasMatch = Regex.Match(s,
                @"^(?<expr>.+?)\s+AS\s+(?<alias>[A-Za-z_][A-Za-z0-9_]*)$",
                RegexOptions.IgnoreCase);

            if (exprAliasMatch.Success)
            {
                list.Add(new ExpressionRef
                {
                    ExpressionSql = exprAliasMatch.Groups["expr"].Value.Trim(),
                    Alias = exprAliasMatch.Groups["alias"].Value
                });
                continue;
            }
...
// Parse 1 nguồn bảng: hỗ trợ "schema.table alias" và "table AS alias"
    ...
    {
        if (idx < 0 || idx + kw.Length > sql.Length) return false;

        // 1. Check previous char (must be boundary)
        if (idx > 0)
        {
            char prev = sql[idx - 1];
        ...
        // 2. Check content
        if (!sql.AsSpan(idx, kw.Length).Equals(kw, StringComparison.OrdinalIgnoreCase)) return false;

        // 3. Check next char (must be boundary)
        if (idx + kw.Length < sql.Length)
        {
            char next = sql[idx + kw.Length];
            if (char.IsLetterOrDigit(next) || next == '_' || next == '$') return false;
        }
...
public class ResolvedQuery
{
    ...
    public Dictionary<string, TableRef> TableAliases { get; set; } = new();
}
...
public class QueryResolver
{
    ...
    {
        return Resolve(node, new Dictionary<string, List<ResolvedColumn>>(StringComparer.OrdinalIgnoreCase));
    }

    private ResolvedQuery Resolve(QueryNode node, Dictionary<string, List<ResolvedColumn>> parentCtes)
    {
        var result = new ResolvedQuery();

        var currentCtes = new Dictionary<string, List<ResolvedColumn>>(parentCtes, StringComparer.OrdinalIgnoreCase);

        if (node.Ctes != null)
        {
            foreach (var kvp in node.Ctes)
            {
                var resolvedCte = Resolve(kvp.Value, currentCtes);
                currentCtes[kvp.Key] = resolvedCte.OutputColumns;
            }
        }

        // [FIX] Phân giải trước các Subquery trong FROM để lấy danh sách cột (ví dụ: FROM (SELECT count(*) as total) t)
        // Giúp tránh lỗi "Column not found" khi SELECT t.total
        foreach (var src in node.FromSources)
        {
            if (src is SubqueryRef sq)
            {
                var subResolved = Resolve(sq.Subquery, currentCtes);
                currentCtes[sq.Alias] = subResolved.OutputColumns;
            }
        }
        ...
        foreach (var item in node.SelectItems)
        {
            switch (item)
            {
                case WildcardRef w:
                    ...
                    {
                        bool usePrefix = node.FromSources.Count > 1;
                        foreach (var src in node.FromSources)
                        {
                            ExpandWildcardForSource(src, aliasMap, outputColumns, usePrefix, currentCtes);
                        }
                    }
                    else
                    {
                        ...
                        ExpandWildcardForTable(tref, outputColumns, false, currentCtes);
                    }
                ...
                case ColumnRef cr:
                    ...
                    {
                        source = FindUniqueColumnOwner(cr.ColumnName, aliasMap.Values.ToList(), currentCtes);
                    ...
                    if (source.Schema == null && currentCtes.TryGetValue(source.Name, out var cteCols))
                    {
                        var col = cteCols.FirstOrDefault(c =>
                            c.OutputName.Equals(cr.ColumnName, StringComparison.OrdinalIgnoreCase));
                        if (col == null) throw new Exception($"Column not found in CTE {source.Name}: {cr.ColumnName}");
                        dataType = col.PgDataType;
                    }
                    else
                    {
                    ...
                    outputColumns.Add(new ResolvedColumn
                    {
                        ...
                        SourceTable = (source.Schema == null && currentCtes.ContainsKey(source.Name))
                            ? null
                            : $"{source.Schema ?? "public"}.{source.Name}",
                ...
                case CastRef cast:
                    ...
                    outputColumns.Add(new ResolvedColumn
                    {
                        ...
                        SourceTable =
                            (sourceTableCastRef.Schema == null && currentCtes.ContainsKey(sourceTableCastRef.Name))
                                ? null
                                : $"{sourceTableCastRef.Schema ?? "public"}.{sourceTableCastRef.Name}",
                        SourceAlias = cast.TableAlias,
                        SourceColumn = cast.ColumnName,
                        PgDataType = cast.CastType
                    });
                    break;
                ...
                case BooleanLiteral b:
                    outputColumns.Add(new ResolvedColumn
                    {
                        OutputName = b.Alias ?? (b.Value ? "true" : "false"),
                        SourceTable = null,
                        SourceAlias = null,
                        SourceColumn = null,
                        PgDataType = "boolean"
        ...
        result.AccessedTables = tables
            .Where(t => !(t.Schema == null && currentCtes.ContainsKey(t.Name)))
            .Select(t => $"{t.Schema ?? "public"}.{t.Name} AS {t.Alias}")
            .Distinct()
            .ToList();
        result.TableAliases = tables
    ...
    private void ExpandWildcardForSource(
        ...
        bool usePrefix,
        Dictionary<string, List<ResolvedColumn>> ctes)
    {
        if (src is TableRef t)
        {
            ExpandWildcardForTable(t, output, usePrefix, ctes);
        }
        else if (src is SubqueryRef sq)
        {
            // [FIX] Tận dụng kết quả đã phân giải ở trên (nếu có) để tránh đệ quy thừa và đảm bảo đồng bộ
            if (ctes.TryGetValue(sq.Alias, out var cachedCols))
            {
                foreach (var c in cachedCols)
                {
                    output.Add(new ResolvedColumn
                    {
                        OutputName = usePrefix ? $"{sq.Alias}_{c.OutputName}" : c.OutputName,
            ...
            var subResolved = Resolve(sq.Subquery, ctes);
    ...
    private void ExpandWildcardForTable(
        TableRef t,
        List<ResolvedColumn> output,
        bool usePrefix,
        Dictionary<string, List<ResolvedColumn>> ctes)
    {
        if (t.Schema == null && ctes.TryGetValue(t.Name, out var cteCols))
        {
            foreach (var c in cteCols)
            {
                output.Add(new ResolvedColumn
                {
                    OutputName = usePrefix ? $"{t.Alias}_{c.OutputName}" : c.OutputName,
                    SourceTable = null,
                    SourceAlias = t.Alias,
                    SourceColumn = c.OutputName,
                    PgDataType = c.PgDataType
        ...
        else
        {
            ...
            foreach (var c in cols)
            {
                output.Add(new ResolvedColumn
                {
                    OutputName = usePrefix ? $"{t.Alias}_{c.ColumnName}" : c.ColumnName,
        ...
        Dictionary<string, List<ResolvedColumn>> ctes)
    {
        ...
        foreach (var t in tables)
        {
            ...
            if (t.Schema == null && ctes.TryGetValue(t.Name, out var cteCols))
            {
                if (cteCols.Any(c => c.OutputName.Equals(column, StringComparison.OrdinalIgnoreCase)))
                    found = true;
            }
            else
            {
